// app/api/re-engagement-email/route.ts
import { NextRequest, NextResponse } from "next/server";

const WEBHOOK_URL = "https://contentlabs.app.n8n.cloud/webhook/re-engagement-email";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFieldMap = {
      ...
      inactivity_period: normalizeString(payload.inactivityPeriod),
      list_size: normalizeString(payload.listSize),
      primary_goal: normalizeString(payload.primaryGoal),
    ...
    const optionalFieldMap: Record<string, string> = {
      incentive: normalizeString(payload.incentive),
      original_value: normalizeString(payload.originalValue),
      sequence_length: normalizeString(payload.sequenceLength),
      previous_attempts: normalizeString(payload.previousAttempts),
      inactivity_reasons: normalizeString(payload.inactivityReasons),
    ...
    console.log("[proxy] Sending POST request to:", WEBHOOK_URL);
